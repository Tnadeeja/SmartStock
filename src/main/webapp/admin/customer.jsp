    ...
    <form method="get" action="customer" class="flex flex-wrap items-center gap-3 flex-grow">
      ...
      <select name="filterAddress" class="border border-primary px-3 py-2 rounded text-dark-blue">
        ...
        <c:forEach var="addr" items="${addressList}">
          <option value="${addr}" <c:if test="${param.filterAddress == addr}">selected</c:if>>${addr}</option>
        </c:forEach>
      </select>

      <button type="submit" class="bg-primary text-white px-4 py-2 rounded hover:bg-primary-dark">Apply</button>
      <button type="button" onclick="window.location.href='customer'" class="bg-gray-500 text-white px-4 py-2 rounded">Clear</button>
    </form>
...
<script>
    ...
    dropdown.classList.toggle("hidden");
     ...
     <!-- Customer Table -->
            <div class="bg-white shadow rounded overflow-x-auto mt-4">
                <table class="min-w-full text-sm text-dark-blue" id="customer-table">
  ...
  <c:set var="totalItems" value="${fn:length(customerList)}" />
