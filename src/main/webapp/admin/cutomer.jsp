   ...
   <tbody>
    <c:forEach var="customer" items="${customerList}" varStatus="loop">
      <tr class="border-t text-center hover:bg-gray-50">
        ...
        <td class="px-4 py-3">${customer.createdAt}</td>
        <td class="px-4 py-3">
        	<div class="flex justify-center gap-2">
          		<a href="outgoing?action=edit&id=${customer.customerId}" class="bg-primary text-white px-3 py-1 rounded hover:bg-primary-dark transition">Update</a>
          		<a href="outgoing?action=delete&id=${customer.customerId}" class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 transition" onclick="return confirm('Are you sure you want to delete this item?')">Delete</a>
        	</div>
        </td>
      </tr>
    </c:forEach>
    <c:if test="${empty customerList}">
      <tr>
        <td colspan="8" class="text-center text-gray-500 py-6">No customer found.</td>
      </tr>
    </c:if>
  </tbody>
</table>
...
<script>
  function showAddForm() {
    window.location.href = '<c:url value="customerForm.jsp?action=add" />';
  }
</script>
<!-- End --> 
    </main>
